...
class OpenRouterClient:
    '''This open router client class was added to use the API key provided by UVA
        OpenRouter (OpenAI-compatible) client for ALL models.'''
    def __init__(self, input_api_key: str, model: str, prompt_template_path: str, prompt_template: str,
        ...
        self.model = model if model else "deepseek/deepseek-chat"

        self.prompt_template_path = prompt_template_path
        self.prompt_template = prompt_template
        self.prompt_job_description_filename = prompt_job_desc_filename or ''
    ...
    def format_messages(self, input_cv: str):
        if self.prompt_template_path.endswith('.json'):
            return format_message_json(
                ...
                input_cv,
                self.prompt_job_description
            )
        return format_message_txt(
    ...
    def __generate_one_cv(self, input_cv: str) -> str:
        output = self.__call_llm(self.format_messages(input_cv))
...
def parse_args() -> argparse.Namespace:
    ...
    provider_group.add_argument(
        "--provider",
        choices=["anthropic", "openai", "together", "deepseek", "openrouter"], #added openrouter
        default="openrouter", #changed from anthropic to openrouter
...
if __name__ == "__main__":
    ...
    if args.provider == 'together':
        ...
        client = DeepSeekClient(input_api_key=user_input_api_key,
                              ...
                              prompt_job_desc_filename=prompt_job_description_name,
                              prompt_job_description=prompt_job_description_str)
    elif args.provider == 'openrouter': #code for openrouter as provider added
        client = OpenRouterClient(
            input_api_key=user_input_api_key,
            model=args.model,
            prompt_template=formatted_prompt_template,
            prompt_template_path=str(prompt_template_path),
            prompt_job_desc_filename=prompt_job_description_name,
